...
const createNewJob = asyncHandler(async (req, res) => {
  ...
  if (
    ...
    return res
      .status(400)
      .json({ message: "Please make sure all fields are filled out" });
  }
  ...
  if (minSalary >= maxSalary) {
    return res
      .status(400)
      .json({ message: "Make sure Max and Min salaries are correct" });
  }
  ...
  if (!job) {
    return res
      .status(400)
      .json({ message: "Error creating a new job, please contact us" });
  }
...
const deleteJob = asyncHandler(async (req, res) => {
  ...
  if (job.status === "Open") {
    return res
      .status(400)
      .json({ message: "Job can not be deleted, make sure it is closed" });
  }
